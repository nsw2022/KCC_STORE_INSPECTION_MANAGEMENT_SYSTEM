   ...
   // 하단의 상세 리스트를 생성하는 함수
    ...
    window.fn_printClick = async function() {
        try {
            ...
            const $inspectionResultList = $('#inspection-result-list'); // 결과를 쌓을 리스트를 jQuery로 선택
            let $temporaryContainer = $('<div class="print-div"></div>');
            const detailButtons = document.querySelectorAll('.detail-btn');
            for (let btn of detailButtons) {
                ...
                for (let editbtn of editButtons) {
                    ...
                    if (editbtn.textContent.trim() === '더보기') { // '더보기' 상태인 버튼만 클릭
                        editbtn.click();
                        console.log(editbtn)
                        await waitForContentExpansion(editbtn); // 각 '더보기' 버튼 클릭 후 콘텐츠가 확장될 때까지 대기
                    }
                    console.log(editbtn)
                }
                await waitForContentExpansion(btn); // 각 '상세보기' 버튼 클릭 후 콘텐츠가 확장될 때까지 대기

                $temporaryContainer.append($inspectionResultList.html());
            }


            // 모든 내용이 확장된 후 PDF 생성
            await generatePDF($temporaryContainer);
            ...
            $('.go-inspection').removeClass('hide');
            $('.edit-btn').removeClass('hide');
            $('.inspection-tabs').removeClass('hide');
            $('.detailed-item').removeClass('flex');
            $('.check-item').removeClass('block');
            $('.inspection-content-wrapper').removeClass('pb-0');
            $('.check-subitem').removeClass('mt-20');
            $('.subitem-info-wrapper').removeClass('mt-20');
    ...
    async function generatePDF(content) {
        ...
        $(element).find('#inspection-result-list > :nth-child(n)').css('display', 'none');
        $(element).find('#inspection-result-list').append(content);
        $('.detail-btn').addClass('hide');
        $('.print-button').addClass('hide');
        $('.go-inspection').addClass('hide');
        $('.edit-btn').addClass('hide');
        $('.inspection-tabs').addClass('hide');
        $(element).find('.detailed-item').addClass('flex');
        $(element).find('.check-item').addClass('block');
        $(element).find('.inspection-content-wrapper').addClass('pb-0');
        $(element).find('.check-subitem').addClass('mt-20');
        $(element).find('.subitem-info-wrapper').addClass('mt-20');

        $(element).find('.check-subitem:last-child').addClass('flex');
        $(element).find('#inspection-result-list').addClass('page-break');
        $(element).find('.check-subitem:not(:first-of-type)').addClass('page-break');
        $(element).find('.subitem-title:not(:first-of-type)').addClass('page-break').addClass('mt-20');

        $(element).find('.item-title:not(:first-of-type)').addClass('page-break');
        $(element).find('.check-subitem').addClass('mt-20');
        $(element).find('.subitem-info-wrapper').addClass('avoid-page-break');
     ...
     */
    function waitForContentExpansion(button, timeout = 10000) {
        return new Promise((resolve, reject) => {
            if (!button) {
