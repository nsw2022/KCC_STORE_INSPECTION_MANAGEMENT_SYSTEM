  ...
  const autocompleteData = {
    store: (includeAll = true) =>
      $.ajax({
        ...
        return includeAll ? ["전체", ...data] : data;
      }),
    ...
    INSP: (includeAll = true) => {
      const data = ["정기 점검", "제품 점검"];
      return Promise.resolve(includeAll ? ["전체", ...data] : data);
    },
    ...
    BottomINSP: () =>
      $.ajax({
        url: "/qsc/inspection-schedule/bottom-INSP-TYPE",
        method: "GET",
      }).then((data) => {
        return data;
      }),
